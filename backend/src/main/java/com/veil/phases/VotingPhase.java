...
public class VotingPhase implements GamePhase {
    ...
    public void submit(GameAction action, GameContext ctx, EventBus bus) {
        // The only command this phase accepts is a vote (Command pattern). Everything else
        // is ignored — night/day actions are illegal here.
        if (action instanceof com.veil.domain.action.VoteAction vote) {
            castVote(vote.actorId(), vote.targetId(), ctx);
        }
    }
